...
Hooks.once("init", async function () {
  ...
  Actors.registerSheet("blades", BladesFactionSheet, {
    types: ["factions"],
    makeDefault: true,
  });
  Actors.registerSheet("blades", BladesClockSheet, {
    types: ["\uD83D\uDD5B clock"],
    makeDefault: true,
  });
  Actors.registerSheet("blades", BladesNPCSheet, {
    types: ["npc"],
    makeDefault: true,
  });
  ...
  Handlebars.registerHelper("crew_vault_coins", (max_coins, options) => {
    ...
    for (let i = 1; i <= max_coins; i++) {
      html +=
        ...
        i +
        '" data-dType="Number" name="system.vault.value" value="' +
        i +
  ...
  Handlebars.registerHelper(
    ...
    function (parameter_name, type, current_value, uniq_id) {
      ...
      html += `<div id="blades-clock-${uniq_id}" class="blades-clock clock-${type} clock-${type}-${current_value}" style="background-image:url('systems/blades-of-the-inquisition/styles/assets/progressclocks-svg/Progress Clock ${type}-${current_value}.svg');">`;
